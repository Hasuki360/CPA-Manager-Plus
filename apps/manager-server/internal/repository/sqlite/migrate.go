...
func Migrate(db *sql.DB) error {
	statements := []string{
		...
		`create table if not exists grok_inspection_runs (
			id integer primary key autoincrement,
			trigger_type text not null,
			status text not null,
			started_at_ms integer not null,
			...
			reauth_count integer not null default 0,
			model_unavailable_count integer not null default 0,
		...
		`create index if not exists idx_grok_inspection_runs_started_at on grok_inspection_runs(started_at_ms)`,
		`create index if not exists idx_grok_inspection_runs_status on grok_inspection_runs(status)`,
		`create table if not exists grok_inspection_results (
			id integer primary key autoincrement,
			run_id integer not null,
			account_key text not null,
			file_name text not null,
			...
			error text,
			created_at_ms integer not null,
			foreign key(run_id) references grok_inspection_runs(id) on delete cascade
		)`,
		`create index if not exists idx_grok_inspection_results_run on grok_inspection_results(run_id)`,
		`create table if not exists grok_inspection_logs (
			id integer primary key autoincrement,
			run_id integer not null,
			level text not null,
			message text not null,
		...
		`create index if not exists idx_grok_inspection_logs_run on grok_inspection_logs(run_id, created_at_ms)`,
